...
## How many member? and How many products (items)? 
```{r}
# How many member
...
## The most 10 important items (Products) for the most 10 important costumers
...
The `whole milk` and `other vegetables`  remain at the first rank. The `yogurt` transactions progress from **5th** position to the **3rd**.  Also we observe a progress for `Sausage` (9->6th), but a consdetable regress of the `root vegetables` from **6th** to **10th** position.

## Apply ```Arules``` algothimes to predict Rules that associate items (Products)
...
With 0.0001 support, 0.9 of confidence and 3 maximums itesm per basket, we obtain **17690** rules.
...
+ This plot shows that the most frequenct fraction of transactions that contain  3 or 4 items (purple, blue) (support).

## Interactive visualisation of the Rules of the most 10 importantitems for the best 1O costumers
